...
import signal
import sys
from blackbox_src.global_vars import GlobalConfig
# ---------------------------------------------------
# Imports
# ---------------------------------------------------
import time
...
from models.make_new_models import CombinedLoss
from tensorflow.keras.datasets import mnist
import threading

# def handle_interrupt(signum, frame):
#     """Handles Ctrl+C and prints the number of queries before exiting."""
#     time.sleep(1)
#     print('Pressed Ctrl+C', file=sys.stderr, flush=True)
#     print(f"\nExecution interrupted! Queries made so far: {GlobalConfig.query_count}",file=sys.stderr, flush=True)
#     sys.stderr.flush()
...
def solve_final_layer(A,B, model,inputShape, lastLayerShape):
    ...
    check_quality(model,len(model.layers)-2, vector[:-1], vector[-1],lastLayerShape,[], do_fix=False)#do_fix=True
...
if __name__=='__main__':
    ...
    model = load_model(args.model, custom_objects={"CombinedLoss": CombinedLoss})
    ...
    random.seed(args.seed) # Sets the seed for Python's built-in random module
    inputShape = model.input_shape[1:] #eg: (784,) for mnist
    hiddenLayerIDs = [i for i in np.arange(1, len(model.layers)-1)] ##eg: [1,2] for mnist784_16x2_1 model ; Creates a list of IDs (indices) for all hidden layers in the model
    neuronsHiddenLayers = [model.layers[i].output_shape[-1] for i in hiddenLayerIDs] # eg:[16,16] Retrieves the output shape of the ith layer in the model
    outputs = model.output_shape[-1]#the number of neurons in the output layer; Returns 1 (e.g., (None, 1) for a 1-class classification model
    extracted_signs = [] #Initializes an empty list to store signs
    ...
    if args.onlySign == False:
        ...
        #
        weights,biases = getWhiteboxSignatures(model, args.layerID) # obtain the actual weights and biases of 2nd layer(with such adjustment with -ve rows)
        ...
        if args.layerID<len(hiddenLayerIDs)+1: # checks if args.layerID falls within the range of valid hidden layer indices (here 1,2 will satisfy)
    ...
    if args.signRecoveryMethod != 'carlini' and not lastLayer: # NeuronWiggle Sign recovery
        ...
        for neuronId in args.tgtNeurons:
            ...
            # stop timer
            stoptime = time.time()
            # total sign recovery time for each neuron
            total_sign_recovery_time=total_sign_recovery_time + (stoptime-starttime)
            ...
            # ---------------------------------------------------
            # printing percentage of corrcetly recovered signs results
            # ---------------------------------------------------
            if whiteIsCorrect: 
                correct_sign_count=correct_sign_count+1
            total_signs = total_signs +1  # Total neurons checked
        ...
        print(f"*********************Total sign recovery time:{total_sign_recovery_time}***********************")
        correct_sign_percentage = (correct_sign_count / total_signs) * 100
        print(f"******************Correctly recovered sign percentage: {correct_sign_percentage:.2f}%**********************")
    ...
    entire_endtime = time.time()
    print(f"****************ENTIRE time to run both sign and signature extraction:{entire_endtime - entire_starttime} *********")
