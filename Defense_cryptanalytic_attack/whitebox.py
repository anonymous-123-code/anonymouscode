...
def getWeightsAndBiases(model, layers):
    ...
    biases = []
    for l in layers:
        w, b = model.get_layer(index=l).get_weights() #uses get_weights function - whitebox model 
        weights.append(np.copy(w))
        biases.append(np.copy(b))
    return weights, biases
#if L1 has weights/bias =w1,b1 and L2 has weights/bias =w2,b2, then weights =[w1,w2] and bias=[b1,b2]
...
def getWhiteboxRealSigns(model, layerID):#(model,2)
    weights, biases = getWeightsAndBiases(model, range(1, layerID + 1)) #(model, (1, and 2)) 
    signsLayer = np.sign(weights[-1][0]) # gets the signs of  the first row elements, a_1 of 2nd layer neurons [0, if w=0, -1 if -ve and +1 if +ve]
    #Changed a bit here so a_k possible if a_1=0
    for i in range(len(signsLayer)):  # finds a non-zero weight if a_1 =0
        c = 1
...
def getWhiteboxSignatures(model, layerID): #eg: (model,2)
    ...
    weights, biases = getWeightsAndBiases(model, range(1, layerID + 1)) #range(1,3) =>1,2

    signsLayer = getWhiteboxRealSigns(model,layerID)# gets the signs of  the first row elements, a_1 of 2nd layer neurons [0, if w=0, -1 if -ve and +1 if +ve]
    weights[-1] = signsLayer[np.newaxis, :] * weights[-1]
    biases[-1] = signsLayer * biases[-1] #This keeps the weights of positive weights intact and reverses the sign of -ve weights (DNY!!!)
    return weights, biases
