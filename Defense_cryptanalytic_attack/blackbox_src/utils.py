...
def matmul(a,b,c,np=np):
    if c is None:
        c = np.zeros(1)
    return np.dot(a,b)+c
def forward(x, A, B, with_relu=False, np=np): #The final output after propagating through all layers
...
def forward_at(point, A, B, d_matrix): #
    ...
    mask_vectors = [layer > 0 for layer in get_hidden_layers(point, A, B)] #computes the intermediate states for CP 'point', gives 0 if -ve and 1 if +ve

    h_matrix = np.array(d_matrix) #d_matrix is the pertubation 
    for i,(matrix,mask) in enumerate(zip(A, mask_vectors)):
        h_matrix = matmul(h_matrix, matrix, None) * mask #gives F(i-1)(x* + δk); x=CP, δk=pertubation
...
def get_hidden_layers(x, A, B, flat=False,np=np): #forward pass through all layers that have been found 
    ...
    """
    if len(A) == 0: return [] #If no weights are provided, return an empty list since there are no layers to process
    region = [] #A list containing the intermediate outputs (pre-activation values) from each layer
    for i,(a,b) in enumerate(zip(A, B)): #Adds a counter to the pairs created by zip(A, B) If zip(A, B) = [(A1, B1), (A2, B2)], then enumerate(zip(A,B))=[(0,(A1,B1)),(1,(A2,B2))]
        x = matmul(x,a,b,np) #forward pass
        if np == jnp:
            region.append(x)
        else:
            region.append(np.copy(x))
        if i < len(A)-1: #relu #ReLU activation is applied to x unless it’s the last layer
            x = x*(x>0)
    if flat:
...
def get_grad(x, direction, weights, biases, eps=1e-6):  #finite difference approximation of the gradient
    #By subtracting the output at (x - eps*direction) from the output at x, you get the change in the model output caused by the small perturbation
    """
    ...
    g1 = (b-a)/eps #(change in output)/input change
    return g1
def get_second_grad_unsigned(x, direction, weights, biases, eps, eps2):
    ...
    """
    #The second derivative is approximated by adding the two gradient values (one for the forward direction and one for the backward direction)
...
def predict_manual_fast(x,weights,biases):
    ...
    orig_x = x
    for i in range(len(weights)):
        x = matmul(x,weights[i],biases[i])#np.matmul(x, weights[i]) + biases[i] #This computes the output value of the NN but doing MAC operations 
        if i<len(weights)-1: # If the current layer is not the last layer, then apply ReLU; not ReLU on last layer
            x = x*(x>0)
    if GlobalConfig.set_save_queries: #If the save queries is set, the query is saved, but this costs a lot of memory 
        GlobalConfig.SAVED_QUERIES.extend(zip(orig_x,x)) #
    return x #gives the f(x)
