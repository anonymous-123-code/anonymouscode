...
def recoverCritPts_Signature(model, weights,biases,layerId,
    ...
    if len(model.layers) == 3:
        PARAM_SEARCH_AT_LOCATION = 1e4 #not sure why a higher value just for #hidden layers=3(DNY!!)
    dimOfInput,dimOfPrevLayer,dimOfLayer = weights[0].shape[0],weights[layerId-1].shape[0],weights[layerId-1].shape[1]
    #number of neurons in the input layer, previous layer of the target, target layer respectively 
    ...
    print("******************************Total ime for signature recovery: ",tSignatureRec)
...
# Redirect stdout to capture print statements while printing to console
    ...
    try:
        ...
        total_error = 0 # taking total error
        total_error_cheat =0 # taking total error after correction
        for i in range(dimOfLayer):
            gaps = []
            ratios = []
            for j in range(dimOfLayer):
                if np.all(np.abs(extracted_normal[:,i])) < 1e-9:
                    extracted_normal[:,i] += 1e-9 #checks if any weight for neuron 0 is very small (close to zero). If it is, we add a small value to avoid division by zero
                ...
                total_error +=np.sum(error) # total error
                error = np.sum(error**2)/np.sum(model.get_layer(index=layer_num+1).get_weights()[0][:,j]**2)
                gaps.append(error)
                ratios.append(ratio)
            
            total_error_cheat +=np.min(gaps) #adding the total error
        ...
        print(f"***********Total signature cheat error: {total_error_cheat:.6f}") # Print the total accumulated error after the loop
        print(f"***********Total signature no cheat error: {total_error:.6f}**************")
        ...
        ###################################################################
        # After you have printed the "Real" and "Corrected" values, calculate the element-wise differences
        # After you have printed the "Real" and "Corrected" values, calculate the element-wise differences
        real_weights = model.get_layer(index=layer_num+1).get_weights()[0]  # Get model weights (first element is the weight matrix)
        real_biases = model.get_layer(index=layer_num+1).get_weights()[1]  # Get model biases (second element)

        #Print the element-wise differences for weights
        #print("\nElement-wise weight differences:")
        element_wise_weight_diff= 0
        element_wise_bias_diff= 0 
        for i in range(real_weights.shape[0]):  # Iterate through rows
            for j in range(real_weights.shape[1]):  # Iterate through columns
                difference = real_weights[i, j] - extracted_normal[i, j]
                # Format the individual elements of the ndarray
                #print(f"Weight at ({i}, {j}): Real = {real_weights[i, j]}, Corrected = {extracted_normal[i, j]}, Difference = {difference}")
                element_wise_weight_diff+=difference

        # Print the element-wise differences for biases
        print("\nElement-wise bias differences:")
        for i in range(real_biases.shape[0]):  # Iterate through biases
            difference = real_biases[i] - extracted_bias[i]
            # Format individual elements of the ndarray
            #print(f"Bias at index {i}: Real = {real_biases[i]:.4f}, Corrected = {extracted_bias[i]:.4f}, Difference = {difference:.4f}")
            element_wise_bias_diff+=difference

        print(f"*****************************Total weight difference: {element_wise_weight_diff}**********************")
        print(f"*****************************Total bias difference: {element_wise_bias_diff}**********************")
        print(f"*****************************Total weight+bias difference:{element_wise_weight_diff + element_wise_bias_diff}******")
        print(f"*****************************#biases: {real_biases.shape[0]}******")
        print(f"*****************************#weights: {real_weights.shape[0]}******")
        print(f"*****************************Percentage of signature error:{((element_wise_weight_diff + element_wise_bias_diff)/(real_biases.shape[0]*(784+1)))*100}******")
        ##################################################################
    finally:
        # Reset stdout
...
#This function computes the pairwise ratio (ak/a1 in the paper), for each compoent, and then finds the one with least error/nans to do the division
...
def graph_solve(all_ratios, all_criticals, expected_neurons, LAYER, debug=False):
    ...
    # Batch them to be sensibly sized #batches of 1000 each 
    ...
    all_pairings = [[] for _ in range(sum(map(len,ratios_group)))] #list containing the length of each batch.
    for batch_index,(criticals,ratios) in enumerate(zip(criticals_group, ratios_group)): 
        # batch_index - index of the batch; (criticals,ratios) - list of CPs and normals of each batch
        print(batch_index)

        # Compute the all-pairs similarity
        axis = list(range(all_ratios.shape[1]))#gives a list of indices, Eg: if there are 5 elements in all_ratios, then axis=[0,1,2,3,4]
        random.shuffle(axis) #Randomly shuffles the list of feature indices; This introduces randomness in selecting which features will be used for similarity computations
        axis = axis[:20] #Extracts the first 20 elements from the shuffled list. These are the randomly selected feature indices to be used for similarity computation
        for dim in axis:
            # We may have an error on one of the directions, so let's try all of them
            scaled_all_ratios =  all_ratios_f32 / all_ratios_f32[:,dim:dim+1] #divides all ratios with the 'dim' column in all_ratios_f32
            ...
            batch_offset = sum(map(len,ratios_group[:batch_index])) #helps calculate the correct global indices for the elements in that batch
            #Adds up these lengths to determine the total number of elements in all the preceding batches.
            ...
            for this_batch_i,global_j in zip(*np.nonzero(np.array(batch_pairings))):
                all_pairings[this_batch_i + batch_offset].append(global_j) #This ensures that all pairings are globally indexed and stored properly
...
def get_ratios_lstsq(model, critical_points, A, B, debug=False, eps=1e-5):
    ...
    for i,point in enumerate(critical_points):
        ...
        column_is_zero = np.mean(np.abs(h_matrix)<1e-8,axis=0) > .5 #gets the mean(out=1 if value is close to zero)
                                                                    #more than half of the elements in that column are close to zero, so marks it zero
        assert np.all((forward(point,A,B, with_relu=True) == 0) == column_is_zero) #This checks whether the forward pass output is exactly zero for each neuron
        # assert ensures that the neurons identified as "inactive" in the forward pass (i.e., their output is zero) match the columns in h_matrix that are identified as "effectively zero" 
        ...
        # Set the columns we know to be wrong (column_is_zero) to NaN so that it's obvious
...
def gather_ratios(critical_points, A ,B ,check_fn, LAYER, COUNT, model,dimInput,dimOfPrevLayer,dimOfLayer, special, eps=1e-6):
    ...
    for point in critical_points:
        if LAYER > 0:
            ...
            if any(np.any(np.abs(x) < 1e-5) for x in get_hidden_layers(point,A,B)): #Maybe adjust to float64 to 1e-12? #check whether the outputs 
                #of any hidden layers before the ReLU activation are smaller than a certain threshold#Small values might lead to numerical instability
                continue #CP ignored
            
        if LAYER > 0 and np.sum(forward(point,A,B) != 0 ) <= 1: #This checks which elements in the output are non-zero and counts them(np.sum)
                                                                #This counts the number of active neurons in the output
        ...
        if len(model.layers) == 3:
            ...
            GRAD_EPS = 1e-4#1e-6#1e-4
        #iteratively compute a normal vector (or ratio) for a given critical point using progressively smaller values of a tolerance parameter, EPS. 
        #If a computation fails, it retries with a smaller tolerance to improve precision
        for EPS in [GRAD_EPS, GRAD_EPS/10, GRAD_EPS/100]:
            try:
                ...
                break #If the computation succeeds, the loop is exited using break, as there’s no need to try smaller values of EPS
            except AcceptableFailure: #If the function get_ratios_lstsq raises an AcceptableFailure exception, the code inside the except block is executed
...
def compute_layer_values(critical_points,model, weights,biases,layerId,dimInput,dimOfPrevLayer,dimOfLayer,special):
    ...
    if LAYER == 0:
        ...
        COUNT = dimOfLayer * np.log(dimOfLayer) * 3  #we need nlogn critical points 
    ...
    partial_biases = None
    A,B = weights[:-1],biases[:-1] #all elements except the last one
    ...
    print()
    print("Start running critical point search to find neurons on layer", LAYER) #we have to find CPs of target layer neurons from the collection of CPs
    ...
    while True:
        ...
        #filters out critical points that are too close to each other
        for i,(ratio1,point1) in enumerate(this_layer_critical_points): #this_layer_critical_points = (normal, point)
            ...
            for args in this_layer_critical_points[i+1:]: # For each point, we compare it with the subsequent points in the list
                try: 
                    ratio2,point2 = args # Unpack the next pair of ratio and point
                except:
                    print("Some error in line compute_layer_values")
                    continue # Skip this iteration if there's an unpacking error
                if np.sum((point1 - point2)**2)**.5 < 1e-10:  #this calculates the Euclidean distance between point1 and point2
                    break # If they are too close, break out of the inner loop
            else:
        ...
        try:                                                #this_layer_critical_points = (normals, points)
            critical_groups, extracted_normals = graph_solve([x[0] for x in this_layer_critical_points], #all the normals corresponding to CPs
                                                             [x[1] for x in this_layer_critical_points], #all the CPs
                                                             dimOfLayer,
                                                             LAYER=LAYER,
                                                             debug=True)
            break
        except GatherMoreData as e: #the graph solving didn't succeed because more data or exploration is needed and raises a GatherMoreData exception
