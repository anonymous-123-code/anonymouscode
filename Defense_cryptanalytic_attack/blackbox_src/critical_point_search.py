...
def do_better_sweep(model, offset=None, direction=None, low=-1e3, high=1e3, dataset=None):
    ...
    shape = model.input_shape[1:] #obtains the input shape excluding the batch size 

    if offset is None:
        offset = np.random.normal(0,1,size=shape).flatten() #obtains a random value between 0 & 1 with input shape -- here (784,)
    if direction is None:
        direction = np.random.normal(0,1,size=shape).flatten() #obtains a random value between 0 & 1 with input shape -- here (784,) a direction
    weights,biases = getAllWeightsAndBiases(model)

    #The "memo_forward_pass" function helps speed up the process by caching the results of the forward pass (model predictions) for specific input values. 
    # If the same input x is encountered again, it retrieves the cached result instead of recomputing it.
    def memo_forward_pass(x, c={}): #just like a cache that stores all the 
        if x not in c:
    ...
    def search(low, high):
        GlobalConfig.crit_query_count+=1 #GlobalConfig is a class that keep track of the query count 
        mid = (low+high)/2
        ...
        if np.abs(f_mid - (f_high + f_low)/2) < SKIP_LINEAR_TOL*((high-low)**.5):
            ...
            if m1 != m2: #there is non-linearity between  low and mid 
                d = (high-low)
                alpha = (y2 - y1 - d * m2) / (d * m1 - d * m2)
                
                x_should_be = low + (y2 - y1 - d * m2) / (m1 - m2) #approximate value of the critical point
                height_should_be = y1 + m1*(y2 - y1 - d * m2) / (m1 - m2) #approx. of f(approximate value of the critical point)
            
            if m1 == m2:
                # If the slopes on both directions are the same (e.g., the function is flat)
                # then we need to split and can't learn anything
                pass #equal to nop - does nothing
                ...
                real_h_at_x = memo_forward_pass(x_should_be) #actual value of f(x_should_be)
